...
<body class="bg-surface font-body text-on-surface islamic-pattern min-h-screen flex p-4 sm:p-6 md:p-8">
    <!-- Main Navigation Suppression: This is a task-focused Transactional Screen -->
    <main class="w-full max-w-[900px] m-auto">
        <div class="bg-surface-container-lowest rounded-xl shadow-[0px_20px_40px_rgba(26,28,28,0.06)] overflow-hidden flex flex-col md:flex-row min-h-[auto] md:min-h-[500px]">
            <!-- Left Column: Illustrative Zone -->
            <div class="md:w-5/12 bg-primary relative overflow-hidden flex flex-col justify-between p-6 md:p-8 text-on-primary">
                ...
                <div class="absolute inset-0 opacity-10 pointer-events-none">
                    <div class="absolute top-[-10%] right-[-10%] w-64 h-64 border-[32px] border-white rounded-full"></div>
                    <div class="absolute bottom-[-5%] left-[-5%] w-48 h-48 border-[24px] border-white rounded-full opacity-50"></div>
                </div>
                <!-- Main Illustration Placeholder -->
                <div class="relative z-10 flex flex-col items-center my-8 md:mt-0 md:mb-0">
                    <div class="w-2/3 max-w-[200px] md:max-w-none md:w-full aspect-[4/5] rounded-xl overflow-hidden shadow-2xl transform -rotate-3 transition-transform hover:rotate-0 duration-500">
                        <img class="w-full h-full object-cover" alt="Kaaba" src="https://lh3.googleusercontent.com/aida-public/AB6AXuCIvnqsKcfyCuXGZl2KkGHIqLUOvXlz1sjyILMF3KpZuEfgkBn8aQAhcDTTQQ70HAqKXYKggV4d5Lt4CmDFMR99XKKo2gWZcKW4Pw_Jdcip03PRe7WsahrxMIuLnSz909IH803iSfyPI6xejQyUwASyedLAIXiOxupuqPYF5vl_7oLsfLRXbpPOKCAAE-4HLYS7Xmjz9OsCxL-uMdHu25XCLz6Fjd0ZtNXLxJDdtnQEzu8lO8iQPkGbuYARFWweC_AmEJLEDI36AWRU" />
                    </div>
                </div>
                <!-- Bottom Branding -->
                <div class="z-10 flex items-center gap-2">
                    <div class="h-1 w-12 bg-white rounded-full"></div>
                    <span class="text-xs font-label font-bold uppercase tracking-widest opacity-80">By Telkomsel</span>
                </div>
            </div>
            <!-- Right Column: Functional Zone -->
            <div class="md:w-7/12 p-6 md:p-8 lg:p-10 flex flex-col justify-center bg-surface-container-lowest">
                <div class="max-w-md mx-auto w-full">
                    ...
                    @if ($errors->any())
                        <div class="mb-6 p-4 bg-error-container rounded-xl">
    ...
    <div id="registerModal" class="hidden fixed inset-0 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
        <div class="bg-surface-container-lowest rounded-2xl shadow-2xl max-w-md w-full max-h-[90vh] overflow-y-auto">
            ...
            <form id="registerForm" class="p-8 space-y-4">
                ...
                <!-- Error Messages -->
                <div id="registerErrors" class="hidden p-4 bg-error-container rounded-lg"></div>
    ...
    <script>
        ...
        document.querySelector('.toggle-pin-visibility')?.addEventListener('click', function(e) {
            e.preventDefault();
        ...
        // PIN Visibility Toggle - Register
        document.querySelector('.toggle-reg-pin-visibility')?.addEventListener('click', function(e) {
            e.preventDefault();
        ...
        // PIN Confirmation Visibility Toggle
        document.querySelector('.toggle-reg-pin-confirm-visibility')?.addEventListener('click', function(e) {
            e.preventDefault();
            const pinInput = document.getElementById('pin-confirm');
            const icon = this.querySelector('.material-symbols-outlined');
            if (pinInput.type === 'password') {
                pinInput.type = 'text';
                icon.textContent = 'visibility_off';
        ...
        function displayRegisterErrors(errors) {
            ...
            if (typeof errors === 'string') {
                errorsDiv.innerHTML = '<p class="text-error font-label text-sm">' + errors + '</p>';
            } else {
                let html = '';
                for (const field in errors) {
                    if (Array.isArray(errors[field])) {
                        errors[field].forEach(msg => {
                            html += '<p class="text-error font-label text-sm">' + msg + '</p>';
                        });
                    } else {
                        html += '<p class="text-error font-label text-sm">' + errors[field] + '</p>';
                    }
                }
